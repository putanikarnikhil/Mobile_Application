...
import { View, Text, TouchableOpacity, ScrollView, Image, TextInput, Alert, StyleSheet, Platform } from 'react-native';
    ...
    useEffect(() => {
        const fetchLocation = async () => {
            ...
            try {
                ...
                const address = fetchedAddress && fetchedAddress.length > 0
...
const TaskDetailPage: React.FC<TaskDetailPageProps> = ({ 
  ...
  const handleCameraUpload = async () => {
    if (!canPerformAction) {
        Alert.alert('Location Required', 'You must have a current location available to take photos.');
        return;
    }

    const { status } = await ImagePicker.getCameraPermissionsAsync(); 
    if (status !== 'granted') {
      Alert.alert('Permission Required', 'Camera permission is required to take photos for task submission. Please grant access in your device settings.');
      return;
    }
  ...
  const handleSubmission = () => {
    if (!canPerformAction) {
        Alert.alert('Location Required', 'Cannot submit without a valid location.');
        return;
    }
    
    if (images.length === 0) {
      Alert.alert("Error", "Please upload at least one photo before submitting.");
      return;
    }
  ...
  const renderDetailItem = (label: string, value: string | number, isStatus?: boolean) => (
    <View style={detailStyles.dataRow}>
        <Text style={detailStyles.dataLabel}>{label}</Text>
        <Text style={[detailStyles.dataValue, isStatus && globalStyles.detailStatusText, isStatus && getStatusBackground(value as Task['status'])]}>
            {value}
        </Text>
    </View>
  );
    ...
    <View style={globalStyles.detailSection}>
      ...
      <View style={globalStyles.gallery}>
        {photoArray.map((uri, index) => (
          <Image key={index} source={{ uri }} style={detailStyles.auditedImage} />
        ))}
      </View>
    </View>
  );
  ...
  const renderAuditedLocation = (submission: Task['submissionData']) => {
    ...
    const formattedDate = submittedDate.toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
    const timeAgo = submittedOn ? `Audited On: ${formattedDate}` : 'Audited On: N/A';

    return (
      <View style={globalStyles.detailSection}>
        <View style={globalStyles.locationHeader}>
          <Ionicons name="location" size={20} color={ColorConstants.primaryAccent} />
          <Text style={globalStyles.locationTitle}>Audited Location</Text>
        </View>
        <Text style={detailStyles.locationText}>
          <Text style={globalStyles.detailLabel}>Latitude:</Text> {loc.latitude}° N
        </Text>
        <Text style={detailStyles.locationText}>
          <Text style={globalStyles.detailLabel}>Longitude:</Text> {loc.longitude}° W
        </Text>
        <Text style={detailStyles.locationText}>
  ...
  return (
    <View style={[{ paddingTop: insets.top, flex: 1 }, globalStyles.detailWrapper]}>
      
      {/* Header */}
      <View style={globalStyles.detailHeaderBar}>
        <TouchableOpacity onPress={onGoBack} style={globalStyles.backButton}>
          <Ionicons name="arrow-back-outline" size={24} color={ColorConstants.darkText} />
        </TouchableOpacity>
      ...
      <ScrollView style={globalStyles.detailScrollView}>
        ...
        {isCompletedOrSubmitted && task.photos.length > 0 && renderAuditedPhotos(task.photos)}
        {isCompletedOrSubmitted && task.submissionData && task.submissionData.auditComment && renderAuditedComments(task.submissionData.auditComment)}
        {isCompletedOrSubmitted && task.submissionData && renderAuditedLocation(task.submissionData)}
        ...
        {!isCompletedOrSubmitted && (
          <>
            ...
            <View style={[globalStyles.detailSection, {paddingBottom: 0}]}>
                <View style={globalStyles.locationHeader}>
                    <Ionicons 
                        ...
                        size={20} 
                        color={canPerformAction ? ColorConstants.primaryAccent : ColorConstants.danger} 
                    />
                    <Text style={globalStyles.locationTitle}>Location Status</Text>
                </View>
                <Text style={[globalStyles.lastUpdatedText, { marginLeft: 30, color: canPerformAction ? ColorConstants.darkText : ColorConstants.danger }]}>
                    {locationStatusText}
                </Text>
                ...
                {canPerformAction && location && (
                    <>
                        <Text style={detailStyles.locationText}>
                            <Text style={globalStyles.detailLabel}>Latitude:</Text> {location.latitude.toFixed(4)}° N
                        </Text>
                        <Text style={detailStyles.locationText}>
                            <Text style={globalStyles.detailLabel}>Longitude:</Text> {location.longitude.toFixed(4)}° W
                        </Text>
                        <Text style={detailStyles.locationText}>
                            <Text style={globalStyles.detailLabel}>Address:</Text> {location.address}
                        </Text>
                    </>
                )}

                {!canPerformAction && !locationLoading && (
                    <Text style={{marginLeft: 30, fontSize: 12, color: ColorConstants.mediumText}}>
            ...
            <View style={globalStyles.uploadSection}>
              <Text style={globalStyles.uploadTitle}>Upload Photos ({images.length} added)</Text>
              
              {/* Camera Upload Button - DISABLED IF NO LOCATION */}
              <TouchableOpacity 
                  style={[globalStyles.uploadBox, !canPerformAction && globalStyles.disabledInput]} 
              ...
              >
                <Ionicons 
                    name="camera-outline" 
                    size={30} 
                    color={canPerformAction ? ColorConstants.primaryAccent : ColorConstants.faintText} 
                />
                <Text style={[globalStyles.uploadText, !canPerformAction && {color: ColorConstants.faintText}]}>
                    {canPerformAction ? 'Tap to Take Photo' : 'Location Required to Take Photo'}
                </Text>
              </TouchableOpacity>
              
              <View style={globalStyles.gallery}>
                {images.map((uri, index) => (
                  <Image key={index} source={{ uri }} style={globalStyles.thumbnailImage} />
                ))}
              </View>
            </View>
            ...
            <View style={globalStyles.commentsSection}>
              ...
              <TextInput
                style={[globalStyles.commentsInput, !canPerformAction && globalStyles.disabledInput]}
                value={comment}
                onChangeText={setComment}
                placeholder={canPerformAction ? "Enter comments for this task submission..." : "Location required to enter comments"}
                multiline
      ...
      {!isCompletedOrSubmitted && (
        <View style={globalStyles.actionButtons}>
          <TouchableOpacity 
            style={[globalStyles.rejectButton, !canPerformAction && globalStyles.disabledButton]} 
            onPress={() => onTaskUpdate(task.id, 'Rejected', images, comment)}
            disabled={!canPerformAction}  
          >
            <Text style={[globalStyles.rejectButtonText, !canPerformAction && {opacity: 0.5}]}>Reject</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            style={[
                globalStyles.submitButton, 
                (locationLoading || !canPerformAction) && globalStyles.disabledButton 
            ]} 
            onPress={handleSubmission} 
            disabled={locationLoading || !canPerformAction} 
          >
            <Text style={globalStyles.submitButtonText}>{locationLoading ? 'Getting Location...' : 'Submit'}</Text>
          </TouchableOpacity>
        </View>
      )}
    </View>
